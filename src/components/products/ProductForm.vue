<template>
  <form class="form" @submit.prevent="onSubmitForm">
    ...
    <span
      class="form__error"
      v-for="error in v$.count.$errors"
      :key="error.$uid"
      >{{ error.$message }}
    </span>

    <VButton class="form__button" type="submit"> Отправить форму </VButton>
  </form>
</template>
  ...
  return {
    name: {
      ...
      minLength: helpers.withMessage(
        `Название не может быть короче ${minLength(3).$params.min} символов`,
        minLength(3)
      ),
    },
    price: {
      ...
      minValueValue: helpers.withMessage(
        `Цена не может быть ниже ${minValue(1).$params.min} `,
        minValue(1)
      ),
    },
    count: {
      ...
      minValueValue: helpers.withMessage(
        `Количество не может быть ниже ${minValue(1).$params.min} `,
        minValue(1)
      ),
    },
  }
